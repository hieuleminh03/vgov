...
public interface ProjectMemberRepository extends JpaRepository<ProjectMember, Long> {
    ...
    @Query("SELECT SUM(pm.workloadPercentage) FROM ProjectMember pm WHERE pm.user.id = :userId AND pm.isActive = true")
    ...
    @Query("SELECT pm FROM ProjectMember pm JOIN FETCH pm.user JOIN FETCH pm.project WHERE pm.user.id = :userId AND pm.isActive = true")
    ...
    @Query("SELECT pm FROM ProjectMember pm WHERE pm.project.pmEmail = :pmEmail AND pm.isActive = true")
    ...
    @Query("SELECT SUM(pm.workloadPercentage) FROM ProjectMember pm WHERE pm.user.id = :userId AND pm.isActive = true AND pm.id != :excludeId")
    BigDecimal getTotalWorkloadByUserIdExcluding(@Param("userId") Long userId, @Param("excludeId") Long excludeId);
    
    @Query("SELECT COUNT(pm.project) FROM ProjectMember pm WHERE pm.user.id = :userId AND pm.isActive = true")
